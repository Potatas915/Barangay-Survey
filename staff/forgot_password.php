...
// system, so identity is confirmed with username + the email on file
// instead of a mailed reset link. Once confirmed, staff set a new
// password right away.

$step = "verify";
$error = "";
$staff_id = $_SESSION["reset_staff_id"] ?? null;
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && ($_POST["action"] ?? "") === "verify") {
    ...
    $stmt = $conn->prepare(
        "SELECT staff_id FROM staff WHERE username = ? AND email = ?"
    );
    ...
    if ($result->num_rows === 1) {
        ...
        $error = "We couldn't match that username and email.";
    }
...
<title>Reset Password &mdash; Staff</title>
...
<div class="portal-card-wrap">
    ...
    <div class="portal-card">
        ...
        <?php if ($step === "verify"): ?>
            ...
            <p class="portal-subtitle">Confirm your username and the email on file.</p>
